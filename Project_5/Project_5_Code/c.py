...
for i in range(len(n_dx)):
    ...
    t = [val1[index1[i]][0], val1[index2][0]]
    for q in range(len(t)):
        ...
        # Plotter analytisk resultat.
        axs2[i].plot(x, u, label='t=%.2f' % (t[q]))

        if q == 0:
            print('For dx=1/%.0f and t_1=%.2f:' % (n_dx[i], t[q]))
            print(np.sqrt((1/len(u[1:]))*np.sum(u[1:] -
                                                val1[index1[i]][2:])**2/(np.sum(u[1:]**2))))
            print(np.sqrt((1/len(u[1:]))*np.sum(u[1:] -
                                                val2[index1[i]][2:])**2/(np.sum(u[1:]**2))))
            print(np.sqrt((1/len(u[1:]))*np.sum(u[1:] -
                                                val3[index1[i]][2:])**2/(np.sum(u[1:]**2))))
            print('----------------------')

        else:
            print('For dx=1/%.0f and t_2=%.2f:' % (n_dx[i], t[q]))
            print(np.sqrt(np.sum(u[1:]-val1[index2][2:])
                          ** 2)/(np.sqrt(np.sum(u[1:]**2))))
            print(np.sqrt(np.sum(u[1:]-val2[index2][2:])
                          ** 2)/(np.sqrt(np.sum(u[1:]**2))))
            print(np.sqrt(np.sum(u[1:]-val3[index2][2:])
                          ** 2)/(np.sqrt(np.sum(u[1:]**2))))
            print('----------------------')

    # Plotting:
    if i == 0:
        axs[0].plot(x, val1[index1[i]][1:], '--',
                    label='dx=1/%.0f and t_1=%.2f s' % (n_dx[i], t[0]))
        axs[0].plot(x, val1[index2][1:], '--',
                    label='dx=1/%.0f and t_2=%.2f s' % (n_dx[i], t[1]))
...
axs2[0].legend()
#fig.tight_layout()
fig.savefig("Output/c.png", dpi=95)
fig2.savefig("Output/b.png", dpi=95)
plt.show()
