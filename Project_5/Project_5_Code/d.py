...
def func_boundary_held(step_fac, n_dx, index1):
    ...
    t = [val2[index1][0], val2[index2][0]] # [Tidspunkt for kurvet linje, for liner]
    ...
    for q in range(len(t)):
        ...
        if q==0:
            ...
            axs[1].plot(x[1:], u[1:]-val2[index1][2:], label='$t_{curve}$=%.2fs' % (t[0]))
            axs[2].plot(x[1:], u[1:]-val3[index1][2:])
        else:
            axs[0].plot(x[1:], u[1:]-val1[index2][2:])
            axs[1].plot(x[1:], u[1:]-val2[index2][2:], label='$t_{linear}$=%.2fs' % (t[1]))
...
def func_boundary_not_held(step_fac, n_dx, index1, stability_constant):
    ...
    index2 = index1 + stability_constant # Denne konstanten matte jeg bare proove meg fram for aa finne riktig index for de ulike dxene.
    ...
    for q in range(len(t)):
        ...
        if q==0:
            axs[0].plot(x[1:], u[1:]-val1[index1][2:])
            axs[1].plot(x[1:], u[1:]-val2[index1][2:], label='t_curve=%.2fs' % (t[0]))
            axs[2].plot(x[1:], u[1:]-val3[index1][2:])
        else:
            axs[0].plot(x[1:], u[1:]-val1[index2][2:])
            axs[1].plot(x[1:], u[1:]-val2[index2][2:], label='t_linear=%.2fs' % (t[1]))
            axs[2].plot(x[1:], u[1:]-val3[index2][2:])
    axs[1].legend()
