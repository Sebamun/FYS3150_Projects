...
# Energi:
# Finner den numeriske verdien for E_m.
E_num = np.array(df.loc[df['T'] == T]['E_tot'])[0]
# Den analytiske verdien for E_m.
...
C_num = beta * E_var**2
C_ana = beta * ((256*np.cosh(8*beta) / Z) -
                (-32*(np.sinh(8*beta)) / Z)**2) / (num_spins**4)
diff_C = np.abs(C_ana-C_num)
#Susceptibilitet:
M_var = np.array(df.loc[df['T'] == T]['M_var'])[0]
Chi_num = beta * M_var**2
# Faar feil svar når jeg tar per spinn her?
Chi_ana = beta * (32*np.exp(8)+128) / Z
