...
void initialize(int, int **, double &, double &, int rand);
// The metropolis algorithm
void Metropolis(int, long &, int **, double &, double &, double *, int &);
// prints to file the results of the calculations
void output(int, int, double, double *, int, int);
...
int main(int argc, char *argv[])
{
    ...
    char *outfilename;
    long idum;
    int **spin_matrix, n_spins, mcs, my_rank, numprocs, n_flips;
    double w[17], average[5], total_average[5],
        initial_temp, final_temp, E, M, temp_step, counter, T_n;
    double cycle_array[1000];
    counter = 0.0;
    n_flips = 0;
    int rand = atoi(argv[8]);
    int exclude_burnin = atoi(argv[9]);
    // Initialverdiene hentes fra en python fil:
    n_spins = atoi(argv[2]);
    mcs = atoi(argv[3]);
    ...
    for (double temperature = initial_temp; temperature <= final_temp; temperature += temp_step)
    {
        ...
        initialize(n_spins, spin_matrix, E, M, rand);
        ...
        {
            Metropolis(n_spins, idum, spin_matrix, E, M, w, n_flips);
            // update expectation values  for local node
            if(exclude_burnin == 0 && cycles > 8e4+myloop_begin){
              average[0] += E;
              average[1] += E * E;
              average[2] += M;
              average[3] += M * M;
            ...
            double *foo = std::find(std::begin(cycle_array), std::end(cycle_array), cycles);
            // When the element is not found, std::find returns the end of the range
            if (temperature == T_n)
            {
                ofile2 << setw(20) << E  << endl;
                if (foo != std::end(cycle_array))
                {
                    //Test som henter ut verdier for energi og magnetisering gitt en temperatur T_n.
                    ofile3 << average[0] / cycles << setw(20) << average[4] / cycles << setw(20) << n_flips << endl; // setw(20) << n_flips << endl;
                }
            }
            //n_flips = 0;
        }
        ...
        if (my_rank == 0)
        {
            output(n_spins, mcs, temperature, total_average, exclude_burnin, numprocs);
        }
    }
...
void Metropolis(int n_spins, long &idum, int **spin_matrix, double &E, double &M, double *w, int &n_flips)
{
    // loop over all spins
    //std::random_device rd;
    //std::mt19937_64 gen(rd());
...
void output(int n_spins, int mcs, double temperature, double *total_average, int exclude_burnin, int numprocs)
{
    double norm;
    if ( exclude_burnin == 0){
      norm = 1 / ((double)(mcs-(8e4*numprocs)));
    }
    else{
      norm = 1 / ((double)(mcs)); // divided by total number of cycles
    }
    ...
    double Evariance = (E2total_average - Etotal_average * Etotal_average); // / n_spins / n_spins / n_spins / n_spins; // Pers spinn.
    double Mvariance = (M2total_average - Mabstotal_average * Mabstotal_average); // / n_spins / n_spins / n_spins / n_spins;
    ofile << setiosflags(ios::showpoint | ios::uppercase);
    ofile << setw(15) << setprecision(8) << temperature;                           // Printes i forste kolonne.
    ofile << setw(15) << setprecision(8) << Etotal_average / n_spins / n_spins;    // Andre kolonne.
    ofile << setw(15) << setprecision(8) << Evariance / temperature / temperature; // Tredje osv....
